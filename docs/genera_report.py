"""Generate the Report PDF (BOZZA) — concise snapshot from data/kpi.json.
Equivalent to mxmap.it/report.html: sintesi, sovranità (donut), provider, settori.
'BOZZA' is drawn on every page."""
import os, json
...
OUTPUT = os.path.join(HERE, "Report_Sovranita_Digitale_PA.pdf")
...
def page_deco(canvas, doc):
    ...
    canvas.rotate(45)
    canvas.drawCentredString(0, -20, "BOZZA")
    canvas.restoreState()
    # top-right BOZZA badge
    canvas.saveState()
    bw, bh = 26 * mm, 8 * mm
    ...
    canvas.drawCentredString(bx + bw / 2, by + 2.3 * mm, "BOZZA")
    # footer
    ...
    canvas.drawString(20 * mm, 10 * mm, "BOZZA — dati preliminari, non per diffusione")
    canvas.drawCentredString(A4[0] / 2, 10 * mm, "Osservatorio Nazionale Sovranità Digitale — CC BY-SA 4.0")
    canvas.drawRightString(A4[0] - 20 * mm, 10 * mm, f"pag. {doc.page}")
    canvas.restoreState()
...
def build():
    ...
    story.append(Paragraph("Stato della Sovranità Digitale<br/>nella PA italiana", s_title))
    story.append(Paragraph("Giugno 2026 — Osservatorio Nazionale Sovranità Digitale · dati MxMap.it su IndicePA", s_sub))
    story.append(HRFlowable(width="100%", thickness=1, color=BLUE, spaceAfter=8, spaceBefore=2))

    # Sintesi
    story.append(Paragraph("In sintesi", s_h2))
    ee = sov.get("extra_eu", {}).get("pct", 0)
    it = sov.get("it", {}).get("pct", 0)
    eu = sov.get("eu_non_it", {}).get("pct", 0)
    story.append(Paragraph(
        f"Sono stati analizzati <b>{fmt(tot.get('n_entities',0))}</b> enti della Pubblica Amministrazione "
        f"italiana registrati nell'IndicePA. Per ciascuno è stato identificato il provider di posta "
        f"elettronica e la <b>giurisdizione</b> a cui è soggetto. Il <b><font color='#D9364F'>{ee:.1f}%</font></b> "
    ...
    if tps:
        story.append(Paragraph("Provider più diffusi", s_h2))
        items = [(p["name"], f"· {fmt(p['count'])} enti", p.get("pct", 0), SOV.get(p.get("sovereignty"), GRAY)) for p in tps]
    ...
    if bc:
        ...
        story.append(Paragraph("Per categoria di ente", s_h2))
        story.append(Paragraph("Quota di enti su provider <b>extra-UE</b>, per categoria (prime 8 per dipendenza).", s_small))
        items = [(c["cluster"], f"· {fmt(c['n_entities'])} enti · {c.get('dominant_provider','')}", c.get("usa_pct", 0), RED) for c in bc]
        story.append(bar_rows(items, label_w=170 * mm))

    # Metodologia + nota
    story.append(Paragraph("Metodologia e note", s_h2))
    story.append(Paragraph(
        "I dati derivano dall'analisi dei record DNS dei domini della PA (IndicePA): per ogni ente si "
        "identifica il provider di posta e la giurisdizione. Dati grezzi di MxMap.it, metodologia aperta e "
        "replicabile, licenza CC BY-SA 4.0.", s_body))
    gen = k.get("generated_at", "")[:10]
    story.append(Paragraph(
        f"<b>BOZZA.</b> Questo documento è una bozza con dati <b>preliminari</b>: la prima misurazione "
        f"ufficiale non è ancora consolidata. Ultimo aggiornamento dati: {gen or 'n/d'}"
        + (f" — copertura {tot.get('coverage_pct',0):.0f}%." if tot.get('coverage_pct') else ".")
        + " Non citare come dato definitivo.", s_small))
